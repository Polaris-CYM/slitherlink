...
def get_force_direction_list(remain):
    ...
    force_direction_list = np.zeros(shape=(nrow+1,ncol+1,4)) # 有(nrow+1,ncol+1)个点，每个点4个方向。或许会有边界的点不能往外走，但这个表示的是必须往哪边走
    # 4个方向分别定义为0-up,1-down,2-left,3-right

    for row_pos in row_force:
        force_direction_list[row_pos[0], row_pos[1], 3] = 1 # (row_pos[0], row_pos[1])这个坐标的点 必须往right走
        force_direction_list[row_pos[0], row_pos[1]+1, 2] = 1 # (row_pos[0], row_pos[1]+1)这个坐标的点 必须往left走
    for col_pos in col_force:
        force_direction_list[col_pos[0], col_pos[1], 1] = 1 # 必须往down走
        force_direction_list[col_pos[0]+1, col_pos[1], 0] = 1 # 必须往up走
...
def constraint_propagation_forward(problem):
    '''
    use the constraint_propagation together wiht forward checking search algorithm to solve the problem
    '''
