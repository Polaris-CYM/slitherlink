...
class Puzzle:
    def __init__(self, rows, cols, cell_values):
        ...
        for r in range(self.rows + 2):
            self.set_board_color(r, 0, 'o')
            self.set_board_color(r, self.cols + 1, 'o')

        for c in range(self.cols + 2):
            self.set_board_color(0, c, 'o')
            self.set_board_color(self.rows + 1, c, 'o')
...
if __name__ == '__main__':
    nrow, ncol, string = generate_string_from_url('http://www.puzzle-loop.com/?v=0&size=4')
    problem = Puzzle(nrow, ncol, string)
    problem.print_slither()

    start_time = time.time()

    print('*******************************************************************')
    print('******************** After solving the slither ********************')
    print('*******************************************************************')
